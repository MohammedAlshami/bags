...
type PanelState = null | { mode: "create" } | { mode: "edit"; item: FaqItem };

export default function AdminFaqItemsPage() {
  ...
  const fetchList = async (): Promise<FaqItem[]> => {
  ...
  const resetForm = () => { setFormQuestion(""); setFormAnswer(""); setFormSortOrder(0); setFormIsActive(true); setPanelError(null); };
  ...
  const savePanel = async () => {
    ...
    try {
      const body = { question: formQuestion, answer: formAnswer, sortOrder: formSortOrder, isActive: formIsActive };
      if (panel.mode === "create") {
        const res = await fetch("/api/admin/faq-items", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
        if (!res.ok) { const d = await res.json().catch(() => ({})) as { error?: string }; throw new Error(d.error || "Failed to create"); }
      } else {
        const res = await fetch(`/api/admin/faq-items/${panel.item._id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  const deleteItem = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/faq-items/${deleteTarget._id}`, { method: "DELETE" });
  ...
  const formFields = (
    <div className="grid gap-4">
      <div>
        <label className="block text-xs text-neutral-500 mb-1">السؤال</label>
        <input value={formQuestion} onChange={(e) => setFormQuestion(e.target.value)} className="w-full border border-neutral-200 px-3 py-2 text-sm" placeholder="السؤال..." />
      </div>
      <div>
        <label className="block text-xs text-neutral-500 mb-1">الإجابة</label>
        <textarea value={formAnswer} onChange={(e) => setFormAnswer(e.target.value)} className="w-full border border-neutral-200 px-3 py-2 text-sm min-h-[100px]" placeholder="الإجابة..." />
      </div>
  ...
  return (
    <div dir="rtl" style={sans} className="relative">
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <h2 className="text-xl font-medium text-neutral-900">الأسئلة الشائعة</h2>
        ...
        <>
          ...
          <aside className="fixed z-[70] flex flex-col bg-white shadow-2xl inset-x-0 bottom-0 max-h-[92vh] rounded-t-2xl border-t border-black/10 md:inset-x-auto md:left-0 md:top-0 md:bottom-0 md:right-auto md:h-full md:max-h-none md:w-full md:max-w-md md:rounded-none md:border-t-0 md:border-l md:border-black/10">
            <div className="flex shrink-0 items-center justify-between gap-3 px-4 py-3">
              <h3 className="text-lg font-medium">{panel?.mode === "create" ? "سؤال جديد" : "تعديل السؤال"}</h3>
      ...
      <div className="rounded-sm border border-black/10 bg-white">
          ...
          <table className="w-full min-w-[32rem] text-sm">
            <thead>
              <tr className="border-b border-black/10 text-right">
                <th className="p-4 font-medium">السؤال</th>
                <th className="p-4 font-medium">الإجابة</th>
            ...
            <tbody>
              {list.map((item) => (
                <tr key={item._id} className="border-b border-black/5 bg-white">
                  <td className="p-4 font-medium truncate max-w-[14rem]">{item.question}</td>
                  <td className="p-4 text-neutral-600 truncate max-w-[20rem]">{item.answer}</td>
                  <td className="p-4">
        ...
        {list.length === 0 && <p className="p-8 text-neutral-500 text-center">لا توجد أسئلة.</p>}
      </div>

      <ConfirmModal
        open={deleteTarget !== null}
        title="حذف السؤال"
        message={deleteTarget ? `هل أنت متأكد من حذف "${deleteTarget.question}"؟` : ""}
        confirmLabel="حذف"
        cancelLabel="إلغاء"
